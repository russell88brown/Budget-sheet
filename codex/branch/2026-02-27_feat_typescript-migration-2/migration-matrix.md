...
| `D04_JournalEngine.gs` | 2048 | 25 | Large mixed engine (I/O + core logic) | Split (high priority) | In progress (2026-02-27): extracted Rule ID assignment, unknown-scenario row-disable, account lookup/validation, policy-row validation, goal-row validation, and income/transfer/expense row validation callbacks to `ts/core`; continue with remaining pure selection/normalization transforms. |

...
| `D04_JournalEngine.gs` | 2108 | 25 | Large mixed engine (I/O + core logic), partially extracted | Split (high priority) | Continue with remaining pure selection/normalization/orchestration transforms; keep sheet I/O in GAS (transfer-row, recurrence-row, and account-row normalization also extracted to TS on 2026-02-27). |
...
- `D04_JournalEngine.gs` has extracted modules for rule IDs, scenario disable, account lookup/validation, policy/goal row validation, income/transfer/expense row validation callbacks, transfer-row normalization, recurrence-row normalization, and account-row normalization.
